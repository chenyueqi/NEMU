...
static PDE kpdir[NR_PDE] align_to_page;						// kernel page directory 页目录项
static PTE kptable[PHY_MEM / PAGE_SIZE] align_to_page;		// kernel page tables 页表项
...
void init_page(void) {
	...
	for (pdir_idx = 0; pdir_idx < PHY_MEM / PT_SIZE; pdir_idx ++) {
		pdir[pdir_idx].val = make_pde(ptable);//将页表的低12位变成全1赋值给相应的页目录项
		pdir[pdir_idx + KOFFSET / PT_SIZE].val = make_pde(ptable);//(2^31 + 2^30) /2^22
